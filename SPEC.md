...
**Hackathon goal:** maximize utilization of **Mistral models/capabilities** by flowing OCR + (default) transcription + optional structured extraction into the same RAG pipeline, while allowing **optional** ElevenLabs STT as an alternate provider.  
**Hackathon strategy note:** prioritize completing core dir2mcp on Day 1, then integrate **native optional x402 payment gating on Day 2** (facilitator-backed) without changing retrieval internals.
**Day-1 scope note:** no TTS required on Day 1 (TTS may be added Day 2).
**Day-2 scope note:** x402 support is optional and targeted for Day 2 only after core retrieval and MCP interoperability are stable.
...
### 6.3 Deletions (hackathon approach)
...
* Exclude obvious secrets/credentials patterns (regexes applied to file **contents**):
  ...
  * **Generic bearer token** (`(?i)token\s*[:=]\s*[A-Za-z0-9\-._]{20,}`): can match innocuous config values named `token` (feature tokens, cache tokens) that are not credentials.
...
### 8.3 Day-1 note on TTS

* TTS is not required on Day 1.
* If added Day 2, it must be additive and not required for core functionality.
...
## 13) Tool set (Day 1 + recommended Mistral-deep)

### 13.1 Day 1 (must ship)
...
### 13.2 Recommended (for hackathon Mistral depth)
...
### 13.3 Optional Day 2 prize track
...
```json
{
  ...
  "required": ["root", "state_dir", "protocol_version", "indexing", "models"]
}
```
...
### 15.10 `dir2mcp.ask_audio` (optional Day 2)

**Description:** same as `ask` but includes audio output (TTS). Not required Day 1.
...
* Recommended Day-2 paid scope: gate `tools/call` (or selected tool names); keep lifecycle (`initialize`, `tools/list`) ungated.
...
## 19) Non-goals (hackathon scope control)
...
* No TTS required on Day 1 (audio output is optional Day 2).
