...
## Hosted Talk-To Runbook (Issue #39)

Use this runbook to execute the full demo from the hosted ElevenLabs talk-to page only.

### 1. Hosted talk-to URL and variables

Keep these values in your operator notes or local env:

- `ELEVENLABS_TALK_TO_URL`: hosted talk-to link copied from ElevenLabs dashboard.
- `ELEVENLABS_AGENT_ID`: stable agent identifier used by the hosted page.
- Optional personalization variables (recommended naming convention):
  - `demo_user_name`
  - `demo_company`
  - `demo_goal`
  - `demo_repo`
...
- Keep values short and presentation-safe (no secrets).
- Use the same key names across prompt, transcript examples, and demo notes.

If your hosted link supports query values, pass them consistently with `agent_id` and the same personalization key names used in the agent prompt.

### 2. Pre-demo smoke checklist
...
```bash
curl -i "$MCP_URL" -X OPTIONS
```

2. Auth token is valid.

```bash
curl -sS -D /tmp/dir2mcp-init.headers \
  -X POST "$MCP_URL" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $DIR2MCP_AUTH_TOKEN" \
  -d '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}'
```

3. Tools are visible with session header from initialize.

```bash
SESSION_ID=$(awk 'BEGIN{IGNORECASE=1} /^MCP-Session-Id:/ {print $2}' /tmp/dir2mcp-init.headers | tr -d '\r')
...
4. Approval settings are correct in ElevenLabs.

- Auto-approve only known read-only tools:
  - `dir2mcp.search`
  - `dir2mcp.ask`
  - `dir2mcp.open_file`
  - `dir2mcp.list_files`
  - `dir2mcp.stats`
- Keep write/mutation-style tools manual or disabled for demo safety.

5. Latency is acceptable for live conversation.

- Target: first tool result within 2-4 seconds for small/medium corpora.
- If sustained latency is higher, reduce query scope (`path_prefix`, smaller `k`) before demo.

### 3. Live-demo fallback playbook

If a tool times out:

1. Say the timeout plainly, then retry once with a narrower query.
2. If retry fails, switch to manual verification via `tools/call` `dir2mcp.search`.
3. Continue the talk track using verified output only.

If a query returns no results:

1. Say "I didn't find evidence for that in this repository."
2. Offer one targeted rephrase.
3. If still empty, close with a clear no-result statement (do not guess).
...
curl -sS \
  ...
  -H "Authorization: Bearer $DIR2MCP_AUTH_TOKEN" \
  -H "MCP-Session-Id: $SESSION_ID" \
  -d '{"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"dir2mcp.search","arguments":{"query":"CORS middleware preflight","k":5}}}'
```

### 4. 3–5 minute hosted demo script (no custom UI)

1. Start server and show endpoint/auth line from `dir2mcp up`.
2. Run `initialize` + `tools/list` once to confirm readiness.
3. Open the hosted ElevenLabs talk-to page (this is the only demo UI).
4. Ask a repository question that triggers an MCP tool-call moment (`search` then `open_file`).
5. Ask one intentionally missing question and show honest no-result behavior.
6. If needed, show manual `curl` verification for credibility and continue.

This script intentionally excludes custom frontend steps and depends only on hosted talk-to + MCP connectivity.
